...
namespace AlexisLefebvre\FixturesBundle\Utils;

use AlexisLefebvre\FixturesBundle\Test\ValidationErrorsConstraint;
...
class HttpAssertions extends TestCase
{
    ...
    public static function assertStatusCode(int $expectedStatusCode, Client $client): void
    {
        ...
        if ($expectedStatusCode !== $client->getResponse()->getStatusCode()) {
            ...
            if ($exception = $client->getContainer()->get('alexis_lefebvre_fixtures.exception_listener')->getLastException()) {
                $helpfulErrorMessage = $exception->getMessage();
            } elseif (
                $client->getContainer()->has('alexis_lefebvre_fixtures.validator') &&
                count($validationErrors = $client->getContainer()->get('alexis_lefebvre_fixtures.validator')->getLastErrors())
            ) {
    ...
    {
        if (!$container->has('alexis_lefebvre_fixtures.validator')) {
        ...
        self::assertThat(
            $container->get('alexis_lefebvre_fixtures.validator')->getLastErrors(),
